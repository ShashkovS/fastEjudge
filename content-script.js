...
function processButtonsTd(butTd, run_id, commentArea, SID) {
    ...
    for (const butObj of [
        ...
        newBut.onclick = () => {
            for (const dumBut of newButtons) {
                newBut.style.background = "";
            }
            butObj.onclick()
                .then(response => {
                    debugger;
        ...
        newButtons.push(newBut)
        butTd.appendChild(newBut);
    }
}

// fetch(`https://server.179.ru/cgi-bin/new-judge?SID=10a2e6abb4590c55&action=91&run_id=787`).then(response => console.log(response));
